...
1. Extracts all unique `genbank_id` values → `rams_taxids.txt`
2. Calls `taxonkit lineage -n -r` → canonical name + rank per TaxID
3. Calls `taxonkit reformat` → populates kingdom/phylum/.../species columns
4. Merges everything back onto the RAMS table and writes the output TSV
